...
for (const [logicalId, resource] of lambdaResources) {
  ...
  if (properties.Runtime !== 'nodejs22.x') {
    fail(`${logicalId} does not use nodejs22.x`);
  }
...
const lambdaRoleIds = new Set(
  lambdaResources.map(([, resource]) => resource.Properties.Role['Fn::GetAtt'][0]),
);
...
if (
  permissionStatements.some(({ actions }) =>
    actions.some((action) => typeof action === 'string' && action.startsWith('kms:')),
  )
) {
  fail('template grants an unapproved KMS action');
}
const ssmStatements = permissionStatements.filter(({ actions }) =>
  actions.some((action) => typeof action === 'string' && action.startsWith('ssm:')),
);
if (ssmStatements.length !== 2) {
  fail(`expected two scoped ssm:GetParameter grants, found ${ssmStatements.length}`);
}
...
for (const { logicalId, statement, actions } of ssmStatements) {
  ...
  if (
    ...
    fail(`unexpected SSM parameter resource: ${JSON.stringify(statement.Resource)}`);
  }
}
if (expectedSsmRoles.size !== 0) {
  fail(`missing SSM permission-bearing roles: ${[...expectedSsmRoles].join(', ')}`);
}
...
for (const [logicalId, table] of tableResources) {
  ...
  if (
    ...
    indexes.some((index, position) => index.IndexName !== expectedIndexes[position])
  ) {
...
function handlerName(logicalId) {
  return logicalId.replace(/LambdaFunction$/, '').replace(/^[A-Z]/, (letter) =>
    letter.toLowerCase(),
  );
}
